        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-14 items-start">
            <div className="lg:col-span-2">
              <p className="text-green-700 font-semibold text-sm uppercase tracking-widest mb-3">{t("joinLabel")}</p>
              <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 leading-tight mb-4">{t("heading")}</h1>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                <p className="text-gray-800 font-semibold text-sm mb-3">{t("helpWith")}</p>
                <div className="space-y-2">
                  {HIGHLIGHT_ICONS.map((icon, i) => (
                    <div key={i} className="flex items-center gap-2.5">
                      <span className="text-lg">{icon}</span>
                      <span className="text-sm text-gray-700">{t(`highlight${i}` as any)}</span>
                    </div>
                  ))}
                </div>
              </div>
